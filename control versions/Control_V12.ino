...
Obstacle obstacleOrder[4] = {RAMP, STAIRS, GIANTS, TREADMILL};
...
const int IRPins[numIRSensors] = {IR_LEFT, IR_FRONT};
...
String modeToString(RobotMode m) {
  switch (m) {
    ...
    case MODE_WALL_TURNING: return "WALL TURNING";
...
void loop() {
  ...
  switch (mode) {
    ...
    case MODE_WALL_TURNING:
      if (millis() - turnStartTime >= TURN_DURATION) {
        stop_Stop();
        mode = MODE_WALL_FOLLOWING;
...
RobotMode check_crossroad() {
    ...
    for (int i = 0; i < numReflectanceSensors; i++) {
      ...
      if (i < numReflectanceSensors / 3) left = true;
      else if (i > 2 * numReflectanceSensors / 3) right = true;
      else middle = true;
    }
...
void wifi_check() {
  ...
  if (packetSize > 0) {
    int len = Udp.read(packetBuffer, 254); // one less than 255 for null-terminator
    if (len > 0) {
    ...
    String content = String(packetBuffer);
    content.trim();
...
void lava_pit() {
  ...
  // stop and extend gripper slowly
  stop_Stop();
  delay(1500);
  int angleGripperMax = 30; // change angle as needed
  for (int i=0; i<angleGripperMax;i++) { 
    gripper.write(i);
    delay(100);
    button_check();
    wifi_check();
  ...
  // drive forward
  int numSecondsLava = 10;
  for (int i=0; i<numSecondsLava; i++) {
    go_Advance_2_wheel(BASE_SPEED, BASE_SPEED);
    delay(1000);
    button_check();
    wifi_check();
  ...
  // once we know on other side, release gripper
  stop_Stop();
  for (int i=angleGripperMax; i>0;i--) { // change angle as needed
    gripper.write(i);
    delay(100);
    button_check();
    wifi_check();
...
void stairs_giants_causeway_ramp_treadmill(int secondsToPass) {
  // faster

  for (int i=secondsToPass; i>0; i--) {
    go_Advance_2_wheel(BASE_SPEED, BASE_SPEED);
    delay(1000);
    button_check();
    wifi_check();
...
void bring_holder_up() {
  int angleHolderMax = 30; // change angle as needed
  for (int i=0; i<angleHolderMax; i++) { 
    holder.write(i);
    delay(50);
    button_check();
    wifi_check();
